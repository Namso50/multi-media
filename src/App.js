...
export default function App() {
    ...
    const [lastUsed, setLastUsed] = useState(new Array(data.length).fill(''))
                ...
                <div style={styles.container}>
                    ...
                    <div style={styles.controlTools}>
                        ...
                        <button style={styles.controlButton}
                            onClick={() => {
                                if (selectedFile) {
                                    setMyFiles(myFiles.filter(file => file.id !== selectedFile.id))
                                    setSelectedFile(null)
                                }
                            }
                            }
                        ...
                        {/* Search */}
                        <input type='text' id='search' placeholder='Search' style={styles.searchInput}
                            onChange={(e) => {
                                ...
                                // when useRegex is not used
                                if (!useRegex) {
                                    let findedFiles = data.filter(file => {
                                        return file.name.toLowerCase().includes(value.toLowerCase())
                                    })
                                    setMyFiles(findedFiles)
                                    return
                                }

                                // when useRegex is used
                                try {
                                    let re = new RegExp(value, "i");
                                    let findedFiles = data.filter(file => {
                                        console.log(file.name, file.path.split(".").reverse()[0])
                                        return re.test(file.name) || re.test(file.path.split(".").reverse()[0])
                                    })
                                    setMyFiles(findedFiles)
                                } catch (error) {
                                    alert("Invalid Regex")
                        ...
                        {/* select to use useRegex or not*/}
                        <button style={styles.isRegexButton} onClick={() => {
                            setUseRegex(!useRegex)
                        }} >{useRegex ? "Disable" : "Enable"} Regex</button>
                            ...
                            {myFiles.map((file) => (
                                <div style={styles.file} className="files" key={file.id} onClick={() => {
                                    if (selectedFile && selectedFile.id === file.id) {
                                        // set last used
                                        const usedFiles = [...lastUsed]
                                        const date = new Date()
                                        usedFiles[file.id] = date.toLocaleString()
                                        setLastUsed(usedFiles)
                                    ...
                                    <p>{file.name}</p>
                                    {lastUsed[file.id] && (<p style={styles.lastUsedText}>last used: {lastUsed[file.id]}</p>)}
                                </div>
                            ))
                            }
                        </div>
